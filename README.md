...
设置页提供手动检查更新入口。当前版本只读取公开 GitHub Releases 元数据，不上传本机配置、搜索词、文件路径、AI Key 或剪贴板内容，也不会自动下载安装器。发现新版本后，应用只提供 Release 页面入口和 MSI 下载链接复制。
...
发布 `v*` tag 或 GitHub Release 时，Actions 会把 MSI 直接上传到 Release assets。普通 push 和 PR 仍只保留 Actions artifact。
